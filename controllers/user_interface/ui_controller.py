...
def ui_songs(id):
    ...
    return render_template(
        "/user_interface/songs.html",
        id=id,
        songs=songs,
        back_button=f"/user-interface/{id}",
    )


# CREATE REQUEST
@ui_home_blueprint.route(
    "/user-interface/<user_id>/request/<song_id>", methods=["POST"]
)
...
def ui_profile(id):
    ...
    requests = request_repository.select_by_user(id)
    return render_template(
        "/user_interface/user/profile.html",
        user=user,
        requests=requests,
        hide_profile=True,
        back_button=f"/user-interface/{id}",
...
def ui_edit_profile(id):
    user = user_repository.select(id)
    return render_template(
        "/user_interface/user/edit-profile.html",
        user=user,
        hide_profile=True,
        back_button=f"/user-interface/{id}/profile",
    )
